...
async function register(registerData) {
    ...
    if (!LETTERS_AND_DIGITS_PATTERN.test(registerData.email)) {
        throw { message: "Email should consist only with English letters and digits!", data: registerData }
    }

    if (registerData.password.length < 5 || !LETTERS_AND_DIGITS_PATTERN.test(registerData.password)) {
        throw {
            message: "Password should be at least 5 characters long, and consist only with English letters and digits!",
            data: registerData
        }
    }
